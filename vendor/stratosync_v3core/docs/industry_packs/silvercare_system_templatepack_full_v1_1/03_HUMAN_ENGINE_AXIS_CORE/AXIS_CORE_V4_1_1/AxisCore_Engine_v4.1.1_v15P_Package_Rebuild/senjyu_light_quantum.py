...
# --- Stellar Origin Patch (fixed soul-goal) ---
try:
    from stellar_origin_patch import apply_fixed_star
except Exception:
    apply_fixed_star = None
...
def run_pipeline(ctx: dict) -> dict:
    # 固定恒星（魂ゴール）を注入（同一出生情報なら毎回同じ）
    if apply_fixed_star is not None:
        try:
    ...
    return {
        ...
        "meta": {
            "engine":"senjyu_light_v1.0",
            "modes":["thoth","modeG","voynich","qpl"],
            "fixed_star": ctx.get("fixed_star"),
            "star_band": ctx.get("star_band"),
        }
    }
